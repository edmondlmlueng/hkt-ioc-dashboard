...
col_video, col_logs, col_genai = st.columns([4, 3, 3])

with col_video:
    st.subheader("📹 CCTV Live Feed")
    st.video("https://www.w3schools.com/html/mov_bbb.mp4", format="video/mp4", autoplay=True, muted=True, loop=True)
...
with col_genai:
    ...
    fig = px.density_heatmap(
        st.session_state.event_logs if "event_logs" in st.session_state else pd.DataFrame(),
        x="X", y="Y",
        nbinsx=10, nbinsy=10,
